...
TOKEN = '6571823614:AAHTiUcrLxkG1glwdTuFzIqS81BhuVuqgQ4'
...
def start_handler(message):
    # Замените 'https://t.me/buy_from_east/429' на вашу реальную ссылку
    link = "<a href='https://t.me/buy_from_east/429'>здесь</a>"
    link1 ="<a href='https://t.me/GooseChooseShop'>goosechooseshop</a>"
    
    # Создаем ReplyKeyboardMarkup с одной кнопкой и устанавливаем параметр resize_keyboard в True
    keyboard = ReplyKeyboardMarkup(resize_keyboard=True)
    
    # Добавляем кнопки в два ряда
    buttons = [
        [InlineKeyboardButton("👟 Наличие в Москве 👜", callback_data='download'), InlineKeyboardButton("🛍️Оформить заказ", callback_data='order')],
    ...
    # Формируем HTML-разметку для сообщения
    html_message = (
        ...
        "<b>Внимание!</b> Все расчеты и прием заказа осуществляются <b>только в боте или у менеджера</b> @goosechooseshop"
    )
    # Отправляем HTML-сообщение с клавиатурой
    bot.send_message(
        message.chat.id,
        html_message,
...
def order_handler(call):
    try:
        ...
        bot.send_message(
            call.message.chat.id,  # Use call.message.chat.id instead of message.chat.id
...
def handle_price_input(message):
    try:
        ...
        selected_category = category_mapping.get(category_number, "Неизвестно")[0]
...
def faq_handler(call):
    ...
    faq_text = (
        "👨💻 Связь с администратором: [GooseChooseShop](https://t.me/GooseChooseShop)\n"
        ...
        "- Отзывы: [Ссылка на чат](@your_reviews_chat_link)"  # Replace @your_reviews_chat_link with the actual link once available
    )
    
    # Creating InlineKeyboardMarkup for the "FAQ" message
    keyboard = InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        InlineKeyboardButton("Для IOS", url='https://apps.apple.com/ru/app/%E5%BE%97%E7%89%A9-%E6%9C%89%E6%AF%92%E7%9A%84%E8%BF%90%E5%8A%A8-%E6%BD%AE%E6%B5%81-%E5%A5%BD%E7%89%A9/id1012871328'),
        InlineKeyboardButton("Для Android", url='https://m.anxinapk.com/rj/12201303.html')
    )
